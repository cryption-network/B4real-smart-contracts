...
describe("StakingPoolUpdatableFixedAPRWhitelisting", function() {
  const depositAmount = parseEther("10");

  let lpTokenInstance, stakingPoolInstance, rewardToken1Instance;
  let startRewardTimestamp, endTimestamp;
  let encodedData;
  let whitelistedDepositor1,
    whitelistedDepositor2,
    whitelistedDepositor1Proof,
    whitelistedDepositor2Proof,
    nonWhitelistedDepositor,
    merkleTree;
  ...
  beforeEach(async () => {
    ...
    whitelistedDepositor1Proof = merkleTree.getHexProof(
      hashToken(whitelistedDepositor1.address)
    );
    whitelistedDepositor2Proof = merkleTree.getHexProof(
      hashToken(whitelistedDepositor2.address)
    );
  });

  it("should revert for non whitelisted depositor", async function() {
    // Transfer LP tokens to depositor
    await lpTokenInstance.transfer(
      nonWhitelistedDepositor.address,
      parseEther("100")
    );
    await lpTokenInstance
      .connect(nonWhitelistedDepositor)
      .approve(stakingPoolInstance.address, depositAmount);
    await expect(
      stakingPoolInstance
        .connect(nonWhitelistedDepositor)
        .deposit(depositAmount, whitelistedDepositor1Proof)
  ...
  it("should handle deposits and withdrawals from multiple users correctly", async function() {
    ...
    const userA = whitelistedDepositor1;
    const userB = whitelistedDepositor2;
    ...
    await stakingPoolInstance
      .connect(userA)
      .deposit(depositAmountA, whitelistedDepositor1Proof);
    ...
    await stakingPoolInstance
      .connect(userB)
      .deposit(depositAmountB, whitelistedDepositor2Proof);
